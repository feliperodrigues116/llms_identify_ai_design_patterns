...
async function openaiChatCompletion(messages, maxTokens = 200, temperature = 0.4) {
  ...
  if (!response.ok) {
    ...
    throw new Errorr(`${response.status} ${response.statusText}\n${JSON.stringify(error)}`);
  }
...
async function processPattern(pattern) {
  ...
  const solutionLLM = await openaiChatCompletion(
  ...
  const analiseLLM = await openaiChatCompletion(
  ...
  const similarityResponse = await openaiChatCompletion(
    [{ role: 'user', content: prompt3 }],
    10, 
    0.0 
  );

  const similarity = similarityResponse.toLowerCase().includes('true') ? 'True' : 'False';
